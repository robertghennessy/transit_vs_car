...
test_log_file = os.path.join(config.logs_dir,'test_SchedulerLog.txt')
...
def main():
    ...
    if os.path.isfile(test_log_file):
        ...
        os.rename(test_log_file, os.path.join(config.logs_dir,
                                                     new_logfile_name))
    # create a new logger 
    logging.basicConfig(filename=test_log_file, level=logging.INFO, 
                        format = '%(asctime)s - %(levelname)s - %(message)s')
    create_tst_csv(csv_file_loc,test_csv_file_loc)
    ptcd.create_job_database(dummy_function, test_csv_file_loc, 
