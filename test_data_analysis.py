...
# round the minimum date down for plotting
datemin = first_date.replace(day=1)
# round the maximum date up for plotting
...
for trip_index in trip_index_list:
    # scheduled trip time
    sched_trip_time = schedule_trips[schedule_trips['trip_index']==trip_index]['sched_trip_duration_secs'].values[0]/60
    ...
    fig.savefig(os.path.join(hist_dir, title_str + '.png'), bbox_inches='tight')
    #plt.show()
    plt.close(fig)
    
    # emperical cumulative density 
    fig = plt.figure()
    plt.plot(sorted_duration_in_traffic, np.linspace(0, 1, len(duration_in_traffic), 
                     endpoint=False))
    plt.xlabel('Trip Duration [minutes]')
    plt.ylabel('ECDF')
    plt.title(title_str)
    fig.savefig(os.path.join(ecdf_dir, title_str + '.png'), bbox_inches='tight')
    #plt.show()
    plt.close(fig)    
    ...
    fig.savefig(os.path.join(time_dir, title_str + '.png'), bbox_inches='tight')
    #plt.show()
    plt.close(fig)
    
    # determine the fraction of trips greater than the train time
    trip_fract = sum(sorted_duration_in_traffic>sched_trip_time)/len(sorted_duration_in_traffic)
